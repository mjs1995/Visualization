...
Types = [ '총인구_여자(명)','총인구_65세이상(명)', '소멸위험지수']
...
for q in Types:
    trace2.append(go.Bar(
        x=df.sort_values(['소멸위험지수'], ascending=False).head(10)['소멸위험지수'],
        y=df.sort_values(['소멸위험지수'], ascending=False).head(10)['sigun_nm'].str.title().tolist(),
        xaxis='x2',
        yaxis='y2',
        marker=dict(
        ...
        visible=False,
        name='Top 10 sigun with the highest {} value'.format('소멸위험지수'),
        orientation='h',
    ))
